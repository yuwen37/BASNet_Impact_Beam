...
factors = [np.mean(factors)] * len(factors)
K_t = []
corner_t = []
tx = []
for i in range(5):
    corner_t.append(np.zeros(len(factors)))
    K_t.append(np.zeros(len(factors)))
...
for img in range(len(factors)):
    ...
    df.index.name = 'index'
    df['x'] = df.index
    df['z'] = df['y'].rolling(400, axis=0, min_periods=None, center=True).mean()
    df['y'] = df['z']
