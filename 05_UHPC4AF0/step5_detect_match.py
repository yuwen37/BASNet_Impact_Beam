...
for img in range(300):
    ...
    ret, target_point[0] = cv2.threshold(target_point[0], 120, 1, cv2.THRESH_BINARY)
    ...
    target_point[0] = morphology.remove_small_objects(target_point[0] > 0, 200, connectivity=1)
    # plt.figure()
    ...
    ret, target_point[1] = cv2.threshold(target_point[1], 100, 1, cv2.THRESH_BINARY)
    kernel = cv2.getStructuringElement(cv2.MORPH_CROSS, (30, 30))
    target_point[1] = cv2.morphologyEx(target_point[1], cv2.MORPH_CLOSE, kernel)
    target_point[1] = morphology.remove_small_objects(target_point[1] > 0, 200, connectivity=1)
    # plt.figure()
...
for i in range(5):
    ...
    edge_txt = pd.DataFrame(edge_y[i])
    edge_txt.to_csv('./results/UHPC{}/edge/edge{}.txt'.format(video_name, str(i + 1)), sep='\n', index=False, header=False)
   # np.savetxt('./results/UHPC{}/edge/edge{}.txt'.format(video_name, str(i + 1)), edge_y[i].reshape(-1), fmt='%.5f')